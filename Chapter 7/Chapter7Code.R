...
## Filtering Rows ----
# Filter the RPG dataset to include only characters with a level greater than 
# or equal to 70
df |>
  filter(Level >= 70) |>
  head()
...
# Filter the RPG dataset to include only characters with a level greater than 
# or equal to 70 and are of class Magician
df |>
...
## Rearranging Rows ----
# Reorder the RPG dataset to display the characters in ascending order 
# of their Level
df |>
  arrange(Level) |>
  head()

# Reorder the RPG dataset to display the characters in descending order 
# of their Level
df |>
  arrange(desc(Level)) |>
  head()
...
# Add a new column to the RPG dataset that contains the average of 
# all attributes of the characters
df |>
  mutate(AvgAttr = (Armor + Weapon + Physical + Magic) / 5) |>
  head()

# Recreate the AvgAtrr column
# Add another column to the RPG dataset that determines if the character is 
# strong or weak based on the AvgAttr column.
# We will use a logical expression to determine if the character is strong 
# or weak.
# If the character's average attributes are greater than or equal to 50, 
# the character is strong, otherwise, the character is weak.
df |>
  mutate(AvgAttr = (Armor + Weapon + Physical + Magic) / 5,
...
# Calculate the average Level and the median Magic power of the characters 
# in the RPG dataset
df |>
  summarize(AvgLevel = mean(Level), MedianMagic = median(Magic))

## Group-wise Operations ----
# Calculate the average level of the characters in the RPG dataset for 
# each class
df |>
  group_by(Class) |>
  summarize(AvgLevel = mean(Level))
...
# Calculate the average level of the characters in the RPG dataset for 
# each class based on whether they beat the final boss or not
df |>
...
# Select the Level, Class, Physical, and FinalBoss columns and rename 
# FinalBoss to FBoss
# Calculate the average Physical strength of the characters for each class
# Add a new column that classifies if a class is Physical dependent based on 
# whether the Class beat the final boss with AvgPhysical less than 50
